...
# data_for_gap <- data[,c("country","level","value")]
# 
...
# 
# data_for_gap_avg <- data_for_gap_wide %>%
...
## OR can we get the individual level difference (-1, 0, 1) 
## and then the country-level difference?
## OR better yet, get the stratified differences
...
data_wide <- data_wealth %>%
  ...
  mutate(
    eq_ancq = anc1_value - ancq_value,
    eq_pncwmq = pncwm_value - pncwmq_value,
    eq_pncnq = pncn_value - pncnq_value
  )
...
# How would we get a negative gap? Eg. Angola highest wealth quintile pncwm-pncwmq
# Did women respond to pncwm quality questions who did not respond to pncwm? Do we need to add !is.na(pncwm) to pncwmq?
ind_dat <- read_csv("/Users/EWilson/Desktop/DAC/QoC_continuum/results/delivery_indicators.dhs_Dec  9.csv")
ind_dat %>%
...
# is this an issue for other countries?
ind_dat %>%
  filter(country == "DRC") %>%
  count(pncwm, pncwmq)
ind_dat %>%
...
# PNCwm wealth equity plot
plot <- ggplot(data_wide, aes(x=country, y=eq_pncwmq)) +
    geom_line(aes(group=country)) +
    geom_point(size=4, aes(color = level)) +
    labs(tag = "(A)",x= "", y = "% PNCwm contact-content gap") +
    # ggtitle(my.title) +
    ylim(-5,25) +
