...
########################################################### SURVEY-WEIGHTED ESTIMATES, COUNTRY, EQUITY STRATIFIED 
indicators <- c('anc1','ancq','pncwm','pncwmq','pncn','pncnq')
# covariates <- c('wealth','urban.rural','age','education','marital.status','first.time.mother','region')
covariates <- c('wealth', 'age', 'first.time.mother')
...
write.csv(strat_svy_data,paste0("/Users/EWilson/Desktop/DAC/QoC_continuum/results/",date,"_data.strat.svy.csv"), row.names = FALSE)
...
svy_data <- svy_data %>% arrange(indicator, value)
